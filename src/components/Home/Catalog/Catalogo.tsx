        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          {categorias.map((cat) => (
            <button
              ...
              className={`relative p-6 rounded-xl border transition-all duration-300 ${categoriaActiva === cat.id 
        ...
        {categoriaSeleccionada && (
          <div className="mt-12 flex flex-wrap justify-center gap-6 fade-sub visible">
            {categoriaSeleccionada.subcategorias.map((sub) => (
              ...
              className={`relative px-6 py-3 text-sm font-medium transition-all duration-300 ${subcategoriaActiva === sub.id
              ? "text-[#6B0F1A]" : "text-neutral-600 hover:text-black"}`}
              >
                
                {sub.nombre}
                
                <span
                className={`absolute left-0 bottom-0 h-[2px] w-full transition-all duration-200 ${subcategoriaActiva === sub.id
                ? "bg-[#6B0F1A] scale-x-100" : "bg-[#6B0F1A] scale-x-0"} origin-left`}
                />
              </button>
            ))}
